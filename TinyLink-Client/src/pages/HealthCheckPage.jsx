...
const HealthCheckPage = () => {
  ...
  const health = useSelector(selectHealthData);
  ...
  // Define the function to dispatch the thunk
  const dispatchFetchHealth = useCallback(() => {
  ...
  useEffect(() => {
    ...
    // Polling logic: Dispatch thunk every 30 seconds
    const intervalId = setInterval(dispatchFetchHealth, 30000);

    // Cleanup state and interval on unmount
    return () => {
      clearInterval(intervalId);
      dispatch(clearHealthData());
    };
  }, [dispatchFetchHealth, dispatch]);
      ...
      return (
        <div className="p-8 bg-red-50 border border-red-300 rounded-xl shadow-lg">
          ...
          <p className="mt-2 text-red-600">
            {error || "Could not connect to the system health endpoint."}
          </p>
          <button
            onClick={dispatchFetchHealth}
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <StatCard
              title="Server Uptime"
              value={
                health?.uptime ? formatUptime(health.uptime) : "Loading..."
              }
